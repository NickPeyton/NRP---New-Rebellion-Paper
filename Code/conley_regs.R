pacman::p_load(sf, tidyverse, stargazer, spatialreg, spatstat, sp,
...
pdf <- pdf %>%
  rename(
    shillPerCapH = shillPerCa,
    shillPerCapL = shillPer_1,
    shillPerCapM = shillPer_2
  )
...
covLabels <- c('ln(Land Owned)', 'ln(Tithe)', 'ln(Alms)', 'ln(Net Income)', 'Friary',
...
stargazer(conley20, 
          ...
          label = 'tab:conley',
          omit=hide_vars, 
          add.lines = list(c('Population', 'Y', 'Y', 'Y', 'Y'),
