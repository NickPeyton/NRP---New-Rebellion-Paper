...
Tests coefficient stability for the elite-grievance gentleman variables across alternative threshold choices. Section 1 (buffer proximity): recomputes binary dummies for snubbed-family (`Family_Snub`), court officer (`Court_Office`), and any gentleman from raw coordinates (`main_gentlemen.csv`) at 10, 15, 20, 25, and 30 km and refits the primary logit at each threshold. Section 2 (IDW specification): recomputes IDW scores for the same three categories at flat-zone radii of 5, 10, 15, 20, and 25 km (w(d) = 1 if d ≤ flat zone, else flat_zone/d) and refits the primary logit at each radius. Section 3 notes that the monastic distance-decay threshold (12.5 km) is baked into the Python notebooks and cannot be varied in R. Outputs `Output/Tables/sensitivity_gent_buffer.tex`, `Output/Tables/sensitivity_gent_idw.tex`, and console coefficient-stability tables.
