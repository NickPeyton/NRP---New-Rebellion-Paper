...
Finally, religious houses in the North differed systematically from their Southern brethren. The average size of a Northern religious house was relatively small, with an average income well below the \pounds200 cutoff for dissolution. In addition, Northern houses spent roughly three times as much as Southern houses on regular alms as a fraction of their income.\footnote{The figures in the \emph{Valor} only record annual alms given from the bequest of a named benefactor, but the extraordinary volume of Northern as opposed to Southern alms at the very least indicates a greater integration into local communities.} Smaller houses had much more local and face-to-face economic interactions than their larger counterparts, potentially creating much more strongly-felt bonds with their local communities. There are many accounts of antagonism between monasteries and their local secular communities, but these come overwhelmingly from large institutions that dominated their local areas and are rarer in the century leading up to the Dissolution. By 1536, changing recruiting practices had substantially reduced the level of conflict in relations between religious houses and their secular neighbors.\autocite[278-9]{Clark2009} These factors, combined with the crucial role of monastic hospitality in facilitating long-distance trade in the sparsely-populated North mentioned by Robert Aske, made monasteries indispensable institutions that facilitated a dignified life for people in a poorer and less-populated region.\autocite[44]{Bush1996}
...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{Output/Images/LittleRebellionDAG.png}
...
& + \beta_{5}TaxLevel_{i} + \beta_{6}Wet1535_{i} +\beta_{7}Wet1536_{i} + \beta_{8}Percy + \beta_{9}Population_{i} \\
& + \beta_{10}\textbf{X}_{i} + \epsilon_i
    \end{aligned}
\end{equation}
Rebellion is one of the three indicators of rebellion discussed above (muster, primary muster, or rebel gentleman), MonasticLand is monastic landownership, Tithe is the value of tithes collected, Alms is the value of recorded almsgiving, MonasticIncome is the value of monastic income net of expenses, TaxLevel is the estimated lay subsidy per capita in 1525, Wet1535 and Wet1536 are Palmer Drought Severity Index values, Percy is a dummy that takes a value of 1 for parishes within 20km of a Percy country seat, Population is the population figure described above, and \textbf{X} is a vector of geographic covariates including x and y coordinates, terrain type, slope, elevation, and distance from the outbreak of the rebellion at Louth Park Abbey. All continuous variables are standardized, so coefficients will reflect the effect of a one standard deviation change in an explanatory variable.

\subsection{Logistic Regressions}

I begin with a simple logistic regression of the rebel muster dummy on the value of monastic land in each parish, then add the other monastic variables, taxation and population, and geographic controls. The coefficient for monastic land ownership is consistently positive and relatively large, but becomes statistically insignificant when geographic controls are added. It is worth noting that the value of alms given is consistently positive, but only occasionally and weakly significant, and decreasingly so as more variables are added. As mentioned above, the alms recorded in the \emph{Valor} are only those given perpetually from the bequest of a named benefactor, so it is possible that the regularity of these alms had become a fixture in local communities or that these bequests indicate a tighter connection with local people, but the evidence is currently insufficient to draw any strong conclusions.

Note that N drops slightly when weather variables are added; this is due to missing data in the Old World Drought Atlas dataset.

\input{Output/Tables/muster_all_total_raw}

\input{Output/Tables/primary_all_total_raw}

\input{Output/Tables/seat_all_total_raw}
